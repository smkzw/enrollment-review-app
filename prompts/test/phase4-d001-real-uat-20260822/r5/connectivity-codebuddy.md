...
Hard boundaries:
- 仅通过模型自身的视觉/浏览器工具操作 `http://127.0.0.1:4271/#/subjects`，视口 1920×1080。
- 禁止读取源码、数据库、服务日志、开发者工具网络响应或后端接口；禁止终端、curl、自写脚本、直接 API、其他 Agent 和模型替换。
- 不修改代码、配置、研究方案或临床原始资料；本轮不确认任何资料上传。
- 只可读取本提示文件。截图只写入 `runs/test/phase4-d001-real-uat-20260822/r5/codebuddy/screenshots/`。

Runner-managed output path: `runs/test/phase4-d001-real-uat-20260822/r5/connectivity/codebuddy.md`。
