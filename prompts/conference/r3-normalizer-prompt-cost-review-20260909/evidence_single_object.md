...
- Do not write the runner-managed report path `runs/conference/r3-normalizer-prompt-cost-review-20260909/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r3-normalizer-prompt-cost-review-20260909_conference_context.md`
- `plans/codex_main_venue_r3-normalizer-prompt-cost-review-20260909.md`
...
Objective:
基于真实耗时与当前源码独立审阅Normalizer提示职责及输入冗余，提出不损失原始证据、缺口、阶段判断的最小降本方案；重点已核实事实少但待核对观察多的输入，给具体实现与验收测试。
...
Output schema:
1. `# Conference Output: r3-normalizer-prompt-cost-review-20260909 - evidence_single_object`
2. `## Output`
