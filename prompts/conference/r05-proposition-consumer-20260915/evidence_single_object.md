...
- Do not write the runner-managed report path `runs/conference/r05-proposition-consumer-20260915/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r05-proposition-consumer-20260915_conference_context.md`
- `plans/codex_main_venue_r05-proposition-consumer-20260915.md`
...
Objective:
只读审阅命题关系消费、方法版本授权、历史身份和工作流接线；原grok审阅终态402额度耗尽，不重试，不算通过；按现行路由进行新独立审阅。仅源码，不修改、不运行测试/模型/数据库。详细范围见 prompts/conference/r05-judgment-content-review-20260914/proposition-evidence-consumer.md
...
Output schema:
1. `# Conference Output: r05-proposition-consumer-20260915 - evidence_single_object`
2. `## Output`
