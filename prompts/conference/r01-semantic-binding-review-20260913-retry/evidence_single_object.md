...
- Do not write the runner-managed report path `runs/conference/r01-semantic-binding-review-20260913-retry/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r01-semantic-binding-review-20260913-retry_conference_context.md`
- `plans/codex_main_venue_r01-semantic-binding-review-20260913-retry.md`
...
Objective:
只读审阅 .trellis/tasks/09-11-e2e-eligibility-review/R01_REVIEW_INPUT_20260913.md 所列当前源代码和设计；重点核实原子条件与证据的通用语义对应，不修改文件、不调用产品模型、不读取私人配置，不自行派发。输出有文件行号的发现及最小完整修复建议。
...
Output schema:
1. `# Conference Output: r01-semantic-binding-review-20260913-retry - evidence_single_object`
2. `## Output`
