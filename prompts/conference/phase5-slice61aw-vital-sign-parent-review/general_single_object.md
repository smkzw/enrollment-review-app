...
- Runner-managed report path: `runs/conference/phase5-slice61aw-vital-sign-parent-review/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/phase5-slice61aw-vital-sign-parent-review_conference_context.md`
- `plans/codex_main_venue_phase5-slice61aw-vital-sign-parent-review.md`
...
Objective:
独立审查 Phase 5.8d D001 II 生命体征最小代表组的项目无关合同与真实 MTPLX 回放。核对原文动作、mandatory/recommended/best_effort 模态、p785 四项生命体征与既有流程目录覆盖关系、p786 条件与审核节点、只读上下文边界、EX-21 合取独立性、三轮修订和双门禁语义；识别任何会造成控制点丢失、模态强化、阶段误绑、重复发布或把技术通过误称临床验收的问题。只读审查，不修改文件，不读取工作区外临床资料。
...
Output schema:
1. `# Conference Participant Output: phase5-slice61aw-vital-sign-parent-review - general_single_object`
2. `## Boundary Check`
