...
- Runner-managed report path: `runs/conference/enrollment-pair-source-review-20260908/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/enrollment-pair-source-review-20260908_conference_context.md`
- `plans/codex_main_venue_enrollment-pair-source-review-20260908.md`
...
Objective:
对冻结SAR产品双模型配对结果进行只读原图抽查，区分真实临床互补、共同错误、缺项、字段粒度与对账局限；不修改产品，不进行新模型测试，不作临床签批。
...
Output schema:
1. `# Conference Participant Output: enrollment-pair-source-review-20260908 - general_single_object`
2. `## Boundary Check`
