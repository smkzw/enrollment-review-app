...
- Runner-managed report path: `runs/conference/phase4-slice46-independent-uat/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/phase4-slice46-independent-uat_conference_context.md`
- `plans/codex_main_venue_phase4-slice46-independent-uat.md`
...
Objective:
由三种独立视觉模型以资深医学监查人员身份对 Phase 4 证据工作流进行清洁环境端到端试用，量化完成率、误操作、遗漏与证据交互，并追查共同根因
...
Output schema:
1. `# Conference Participant Output: phase4-slice46-independent-uat - visual_pi_k3_256k`
2. `## Boundary Check`
