...
- Do not write the runner-managed report path `runs/conference/r05-shared-control-input-20260914/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r05-shared-control-input-20260914_conference_context.md`
- `plans/codex_main_venue_r05-shared-control-input-20260914.md`
...
Objective:
只读审阅跨章节要求从正式发布到双读条款包的新接线，查历史哈希、来源节点、忽略要求和语义失真；不测试、不调用产品模型或数据库
...
Output schema:
1. `# Conference Output: r05-shared-control-input-20260914 - evidence_single_object`
2. `## Output`
