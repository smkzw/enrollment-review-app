...
- Do not write the runner-managed report path `runs/conference/r3-normalizer-latency-source-review-20260909/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r3-normalizer-latency-source-review-20260909_conference_context.md`
- `plans/codex_main_venue_r3-normalizer-latency-source-review-20260909.md`
...
Objective:
只读审阅产品规范化提示重复、来源校验失败反馈及耗时，提出不损失来源和待核对内容的最小优化方案；不调用模型API、不修改代码、病例或默认模型。
...
Output schema:
1. `# Conference Output: r3-normalizer-latency-source-review-20260909 - evidence_single_object`
2. `## Output`
