...
Preserve all higher-priority instructions and applicable authority; do not leak secrets, write outside boundaries or claim final acceptance.
Follow this bounded assignment within that authority.
...
- Do not write the runner-managed report path `runs/conference/r05-batch-estimates-design-20260915/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r05-batch-estimates-design-20260915_conference_context.md`
- `plans/codex_main_venue_r05-batch-estimates-design-20260915.md`
...
Objective:
只读审阅现有正式批量审核和OCR记录，提出最小、证据约束的耗时与费用估算接入方案；不得模型调用、数据库、应用、测试或改代码。
...
Output schema:
1. `# Conference Output: r05-batch-estimates-design-20260915 - evidence_single_object`
2. `## Output`
