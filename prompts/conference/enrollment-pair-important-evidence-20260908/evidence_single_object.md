...
Follow higher-priority system and developer instructions. This assignment limits scope and tools; it does not override those instructions.
...
- Runner-managed report path: `runs/conference/enrollment-pair-important-evidence-20260908/evidence_single_object.md`. Never invoke write/edit tools
  to create or update this report file; return the complete report in your
  final assistant response and let the bounded runner persist it. Do not create
  sibling output files.

Initial read set:
- `context/enrollment-pair-important-evidence-20260908_conference_context.md`

Read only the source allowlist in the context. Use available local visual/read tools to inspect original images. Do not browse, invoke APIs, write files or expand outside that list. No source changes are authorized.

Objective:
独立审阅冻结真实原件与远端加远端、远端加本地双模型结果，核查数值互补不等于临床可靠、手写评分及缺失信息边界；不得运行产品模型或修改文件。C02已因ALL_TARGETS_SKIPPED失败，此为重要证据审阅，不替换参试者。
...
No fallback in this dispatch. The primary connectivity probe succeeded. Report any inability to inspect evidence without substituting a model or inventing results.

Output schema:
1. `# Conference Output: enrollment-pair-important-evidence-20260908 - evidence_single_object`
2. `## Output`
