...
- Runner-managed report path: `runs/conference/r05-formal-publication-actions-20260914/evidence_single_object.md`. Never invoke write/edit tools
...
- `app/services/control_action_publication.py`
- `app/domain/contracts/control_action_origin.py`
- `app/domain/control_action_directives.py`
- `app/domain/contracts/review.py` ActionRequest and relevant base serializers
- `app/storage/control_action_validation.py`
- `app/storage/review_control_repository.py`
- `app/storage/models.py` ActionRequestRecord
- `app/storage/repositories.py` _check_action_scope, _action_columns, ActionRequestRepository and run storage
- `app/storage/migrations/versions/0027_action_control_origin.py`
- `app/services/review_history_service.py`
- `app/services/review_action_command.py`
...
- `frontend/src/components/review/ReviewActionResponseDialog.tsx`
- `context/r05-formal-publication-actions-20260914_conference_context.md`
- `plans/codex_main_venue_r05-formal-publication-actions-20260914.md`

The initial read set is not a blanket prohibition on additional tool calls or evidence. If more context is required, obtain it with the available tools, explain why, and record what was read or changed.

Objective:
只读源码审阅V2正式整批保存与既有ActionRequest扩展控制来源的全链。禁止测试、应用import、DB读写、产品调用、递归派发和任何文件编辑。检查原子性/幂等/历史不改写、旧载荷哈希兼容、控制来源/强度、报告办理联动、method-adoption与资格授权边界。正式HTTP入口未启用、评测批准签发未实现、临床语义完整性仍开放，不把保守UNKNOWN当完成。指出真实代码缺陷并给最小修复方案，按文件行号。
...
Output schema:
1. `# Conference Output: r05-formal-publication-actions-20260914 - evidence_single_object`
2. `## Output`
