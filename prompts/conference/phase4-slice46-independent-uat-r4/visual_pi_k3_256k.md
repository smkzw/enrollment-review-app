...
- Runner-managed report path: `runs/conference/phase4-slice46-independent-uat-r4/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/phase4-slice46-independent-uat-r4_conference_context.md`
- `plans/codex_main_venue_phase4-slice46-independent-uat-r4.md`
...
Objective:
以真实医学监查人员视角，在清洁隔离的正式本地系统中端到端验收 Phase 4 资料上传、处理、OCR核对、被提及资料、生成启用、原件滚动与重点标注、中文交互及宽屏视觉；测试者必须使用系统自身功能，不替代系统独立Agent作临床判断。
...
Output schema:
1. `# Conference Participant Output: phase4-slice46-independent-uat-r4 - visual_pi_k3_256k`
2. `## Boundary Check`
