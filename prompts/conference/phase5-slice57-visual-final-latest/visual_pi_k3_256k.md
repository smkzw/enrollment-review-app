...
- Runner-managed report path: `runs/conference/phase5-slice57-visual-final-latest/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/phase5-slice57-visual-final-latest_conference_context.md`
- `plans/codex_main_venue_phase5-slice57-visual-final-latest.md`
...
Objective:
以懒惰但专业的中文临床试验医学监查员视角，独立审查 Phase 5.7 Patient Profile 人工事实修订、历史版本对比、原文回源和宽屏交互；重点识别误导、版本混用、信息层级、中文原生性、1080P至4K视觉效率与可恢复性问题
...
You must use visual/image inspection tools to open every screenshot listed in the conference context at 1080P, 2K, and 4K. If browser or Playwright control is available, you may also exercise the corresponding read-only interaction, but screenshots are the minimum evidence. Evaluate the interface as a time-poor but clinically sophisticated native-Chinese medical monitor: identify whether the change, reason, source, affected scope, historical Profile version, and evidence return path can be understood quickly and without version confusion. Report each finding with viewport, exact region, observable evidence, user consequence, severity, and concrete remediation. Explicitly state when no defect is visible. Do not modify source or regenerate screenshots.
...
Output schema:
1. `# Conference Participant Output: phase5-slice57-visual-final-latest - visual_pi_k3_256k`
2. `## Boundary Check`
