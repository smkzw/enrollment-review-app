...
- Agent/provider/model assigned by Codex: `pi` / `opencode-go` / `muse-spark-1.2-contributor`
...
- Runner-managed report path: `runs/conference/phase5-slice58-cross-section-controls-20260824/general_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
你必须实际阅读上下文列出的领域合同、当前流程表目录构建器和发布门禁，从临床规则建模与可追溯性角度审查：除官方 IN/EX 和流程表操作外，如何从全文章节发现并冻结与预筛/筛选/导入/基线入排相关的禁限用药、洗脱期、必做评估、时间窗、例外与研究者判断。每个正式控制点必须结构化表达“何时/哪个审核节点”、“应做或核对什么”、“应达到什么条件”、“禁止什么事件或暴露”、“例外/时间锚点/最低证据”，并处理同一要求在多章节中的补充、重复与冲突。必须比较“扩展现有必做项目录”与“新建独立方案审核控制点目录”的优缺点，给出稳定身份、来源闭包、去重、冲突、完整性和发布停止条件。不得建议使用 D001/MG 特异硬编码。
...
Output schema:
1. `# Conference Participant Output: phase5-slice58-cross-section-controls-20260824 - general_pi_qwen38`
2. `## Boundary Check`
