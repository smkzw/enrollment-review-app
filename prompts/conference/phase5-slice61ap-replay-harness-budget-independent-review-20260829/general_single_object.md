...
- Do not write the runner-managed report path `runs/conference/phase5-slice61ap-replay-harness-budget-independent-review-20260829/general_single_object.md`; return the complete report for the runner.

Initial read set:
- `context/phase5-slice61ap-replay-harness-budget-independent-review-20260829_conference_context.md`
- `plans/codex_main_venue_phase5-slice61ap-replay-harness-budget-independent-review-20260829.md`

Objective:
只读独立审查 Phase 5.8d 模型无关方案重放 harness 与串行修订预算合同。挑战原始 DOCX 到稳定 source_ref 包的真实产品链、人工矩阵及项目特异依赖隔离、来源哈希和时间钉住、本机路径泄漏、整包外部指纹、双运行可重现性、全局修订预算与无进展终止。核对 Codex 修订后的实际代码与测试，不运行临床模型，不发布控制点，不把 DOCX-only 写成 PDF 已支持。
...
Output schema:
1. `# Conference Participant Output: phase5-slice61ap-replay-harness-budget-independent-review-20260829 - general_single_object`
2. `## Boundary Check`
