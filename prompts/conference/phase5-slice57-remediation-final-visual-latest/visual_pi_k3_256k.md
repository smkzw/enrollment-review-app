...
- Runner-managed report path: `runs/conference/phase5-slice57-remediation-final-visual-latest/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/phase5-slice57-remediation-final-visual-latest_conference_context.md`
- `plans/codex_main_venue_phase5-slice57-remediation-final-visual-latest.md`
...
Objective:
以懒惰但专业的中文临床试验医学监查员视角，独立审查 Phase 5.7 修订后的 Patient Profile 人工事实修订、生成后档案历史、原文证据定位一致性、草稿保留和 1080P/2K/4K 宽屏交互；识别任何误导、版本混用、错误红框、信息层级、中文原生性、可恢复性与视觉效率问题。仅审阅，不修改代码。
...
Output schema:
1. `# Conference Participant Output: phase5-slice57-remediation-final-visual-latest - visual_pi_k3_256k`
2. `## Boundary Check`
