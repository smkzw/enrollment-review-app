...
Preserve all applicable higher-priority instructions and the delegated read-only boundary.
This prompt bounds the assignment; it does not override higher-priority authority.
...
- Do not write the runner-managed report path `runs/conference/r05-observation-selection-20260915/evidence_single_object.md`; return the complete report and let the runner persist it.
- Do not claim final clinical, regulatory, visual, browser, or user-facing acceptance authority; Codex remains final authority.

Initial read set:
- `context/r05-observation-selection-20260915_conference_context.md`
- `plans/codex_main_venue_r05-observation-selection-20260915.md`
...
Objective:
只读设计审阅：基于现有R3来源资格/命题/确定性计算，补齐官方及跨章要求的有限观察集合、最近/最早与复查选择通用闭环，提出最小完整实现，不把永久UNKNOWN或新未消费合同当完成。
...
Output schema:
1. `# Conference Output: r05-observation-selection-20260915 - evidence_single_object`
2. `## Output`
