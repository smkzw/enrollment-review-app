...
- Runner-managed report path: `runs/conference/enrollment_phase2_acceptance/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/enrollment_phase2_acceptance_conference_context.md`
- `plans/codex_main_venue_enrollment_phase2_acceptance.md`
...
Objective:
以真实医学监查员与独立工程审查者视角，验收Phase 2 SQLite领域持久化、任务恢复、接口中文表达及Phase 1.5界面无回归，明确区分已实现与未实现能力
...
Output schema:
1. `# Conference Participant Output: enrollment_phase2_acceptance - visual_pi_k3_256k`
2. `## Boundary Check`
