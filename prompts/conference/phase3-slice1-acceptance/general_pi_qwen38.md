...
- Runner-managed report path: `runs/conference/phase3-slice1-acceptance/general_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/phase3-slice1-acceptance_conference_context.md`
- `plans/codex_main_venue_phase3-slice1-acceptance.md`
...
Objective:
独立审查 Phase 3 第一切片的方案原文登记、DOCX结构提取、受控渲染、来源对齐、不可变存储和真实方案回归，寻找会导致临床规则错漏或虚假来源定位的系统性缺陷；仅审查，不修改文件。
...
Output schema:
1. `# Conference Participant Output: phase3-slice1-acceptance - general_pi_qwen38`
2. `## Boundary Check`
