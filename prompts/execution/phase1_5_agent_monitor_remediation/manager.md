...
- Role id: `complex_manager_cursor`
- Provider/model: `cursor-cli` / `auto`
- Role description: execution manager for other complex work; Cursor CLI auto refines the implementation plan, inspects worker outputs, resolves blockers, and requests targeted reruns; no conference
...
- Runner-managed report path: `runs/execution/phase1_5_agent_monitor_remediation/manager.md`. Never invoke write/edit tools
...
- `runs/execution/phase1_5_agent_monitor_remediation/worker_01.md`
- `runs/execution/phase1_5_agent_monitor_remediation/worker_02.md`
- `runs/execution/phase1_5_agent_monitor_remediation/worker_03.md`
...
Review the actual git diff and test output, not worker confidence. Reject single-subject patches, hidden filters, clinical conclusion derivation inside presentation components, raw internal labels, or claims that synthetic Profile data proves real extraction. Check for overlapping edits in `WorkbenchPage.tsx`, generated-fixture/source drift, and tests that merely lock in old incorrect wording. You may run read-only tests and apply only a narrowly necessary integration repair after recording it.
...
Output schema:
1. `# Execution Output: phase1_5_agent_monitor_remediation - complex_manager_cursor`
