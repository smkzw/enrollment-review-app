...
- Task id: `phase5-slice58d-compact-phase-wire-20260825`
- Role id: `worker_03`
...
- Runner-managed report path: `runs/execution/phase5-slice58d-compact-phase-wire-20260825/worker_03.md`. Never invoke write/edit tools
...
Objective:
以 provider wire 分组压缩消除期别语义重复输出，同时保持逐单元领域结果和严格来源门禁。

Task:
Execute only this assigned work item: 用修复后的 D001 内存清单对真实首批及一个非元数据批次运行本地系统 Agent，比较输出字符、耗时、逐单元结论和 gate。
...
Output schema:
1. `# Execution Output: phase5-slice58d-compact-phase-wire-20260825 - worker_03`
