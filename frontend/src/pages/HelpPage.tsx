...
const TRIAL_HELP_SECTIONS: ReadonlyArray<HelpStep> = [
  ...
  },
  {
    ...
    steps: [
      ...
      "只补交新资料时选择「补充资料」：系统沿用上一份有效资料集合并加入本次文件；重新提供本节点全部资料时选择「建立完整资料版本」：本次文件构成新的完整集合，旧版本仍保留供追溯。",
      ...
      "红框只表示系统能够精确对应原件中的具体区域。若页面只显示原文文字、页内摘录或页码，系统会说明可定位的范围，不会绘制推测的红框。",
  ...
  },
  {
    ...
    steps: [
      ...
      "每条证据显示资料文件、页码、可定位范围及原因；「仅页码」表示只能打开所在页，不能框出具体文字。",
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      "重新解构：先在首页明确选择目标正式项目，再上传同一项目的新版 DOCX 方案。系统会把当前正式版本与新草稿并列，并按官方编号逐条显示规则新增、删除及原文、逻辑、时间要求、例外条件、所需资料和应完成阶段的变化。",
      "重新解构时上传的新版方案只允许更新版本、日期与文件内容；方案编号和研究期别必须与目标项目一致，否则会阻止发布并提示原因。",
      "基于反馈修订：如果系统理解方案原文有误，请先选择对应的官方规则，再说明具体问题；系统只重做这一条规则并生成修订稿。补充解释说明只作为审阅说明保存，不会改写规则。",
      "手工修订：可调整所选子项现有的判断关系、阈值、时间要求、例外关系、所需资料和应完成阶段。方案原文及来源定位始终只读；需要增加、删除或重组条件时，请改用「说明问题并重新解构」。",
      "每次修订后都要查看完整性检查和方案原文定位。存在官方编号不一致、来源缺失、逻辑不完整或项目不匹配时，发布按钮会保持不可用，并明确显示需要处理的事项。",
  ...
  },
  {
    ...
    steps: [
      ...
      "规则变化的含义：中间栏显示当前正式版本（左）与新草稿（右）在原文、逻辑、时间要求、例外条件、所需资料或应完成阶段上的前后变化，逐条核对后再决定是否接受。",
...
const FORMAL_HELP_SECTIONS: ReadonlyArray<HelpStep> = [
  ...
  },
  {
    title: "上传资料与核对原件",
    steps: [
      "该节点还没有当前有效资料时，先选择「建立完整资料版本」；已有当前有效资料时，才可选择「补充资料」。",
