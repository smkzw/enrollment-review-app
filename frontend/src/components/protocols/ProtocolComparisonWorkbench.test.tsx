...
describe("重新解构并列差异工作台", () => {
  ...
  it("展示完整性结论和所选规则的方案依据", () => {
    renderWorkbench();
    expect(screen.getByText("可以发布")).toBeInTheDocument();
    expect(screen.getByText(/IN-01 的方案依据/)).toBeInTheDocument();
    expect(screen.getAllByText(/年龄/).length).toBeGreaterThan(0);
  });
  ...
  it("完整性未通过时禁止发布", () => {
          ...
          issues: [
            {
              ...
              checkName: "来源定位",
              level: "阻止发布",
              problem: "一条规则缺少方案原文定位",
              impact: "目前不能发布",
              nextAction: "补充原文定位后重新检查",
              affectedRefs: ["IN-01"],
              repairScope: ["IN-01"],
            },
          ],
    ...
    expect(screen.getByText("暂不能发布")).toBeInTheDocument();
  });
});
