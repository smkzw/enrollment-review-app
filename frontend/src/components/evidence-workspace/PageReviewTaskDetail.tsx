...
export function PageReviewTaskDetail({ subjectId, episodeId, jobId }: { subjectId: string; episodeId: string; jobId: string }) {
  ...
  async function open(pageIndex: number) {
    ...
    catch { setError("复核暂未开始，请稍后重试。原有读法和资料保留。"); }
    finally { setOpening(false); }
  }
  if (auxiliaryId !== null) return <>
    <button className="button" onClick={() => setAuxiliaryId(null)}>返回页面判读概况</button>
  ...
  if (detail.state.status === "error") return <ErrorState message="暂时无法读取资料判读记录。" onRetry={detail.retry} />;
  ...
  return <section className="tasks-section">
    ...
    <h3>原件内容分歧</h3>
    {conflicts.length === 0 ? <p>{status.reviewStatus === "processing" ? "资料仍在判读中。"
      : status.reviewStatus !== "ready" ? "资料判读尚未完成，暂不能据此确认是否存在读法分歧。"
      : "本次未发现需再次核对的数值、日期或手写内容。其他待核实事项仍保留。"}</p>
      : <ul className="persistent-task__files">{conflicts.map((item) => <li key={item.pageIndex}>
        <div><strong>{item.fileName} · 第 {item.pageNumber} 页</strong><span>{item.fieldCount} 项读法不同</span></div>
