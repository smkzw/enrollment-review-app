...
## 阶段状态（2026-09-22源码核对，非临床验收）

| 阶段 | 状态 |
|---|---|
| Phase0 / 0.5 / 1 | 历史已完成并归档，不等于本轮全功能重验 |
| Phase1.5 | 历史角色UAT方式获批；最终实际大屏验收仍须按当前版本验证 |
| Phase2 / 3 / 4 | 草稿/控制目录共同发布和原图/OCR骨架已有；非法evaluation被改研究者判断、完整覆盖/主来源链须W0/W2/W3核修；不把旧归档当本轮完成 |
| Phase5 | 事实/事件/用药/Profile已有；新来源证明和更正失效待W3/W5，完整原件QC未通过 |
| Phase5.5 | 已有双读及来源政策合同；运行主入口仍强制双读，V3主OCR/局部核实尚未贯通；W3替换默认，不恢复历史模型数量验收 |
| Phase6 | 工作台、复杂资格、冻结计算已有；工作稿未共享整条件选择/完整跨章范围，W1/W4待修 |
| Phase7 | 正式行动/冻结报告/更正服务存在；同观察影响、补证和历史保持W5/W6待验证；本轮未查库，不断言无实体 |
