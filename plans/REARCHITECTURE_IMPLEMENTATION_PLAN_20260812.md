...
**状态：** 已批准持续实施，接回工程复审后修订；尚未临床验收。
**设计基线：** R3临床设计 + 2026-09-12用户指定组合与泛化要求。
**原则：** 每阶段有产物与验收证据，未达退出条件不宣布完成；修复跨阶段共同问题可提前，不绕过临床验收。已授权工作连续执行，阶段复盘不是自动暂停点。

## 阶段状态（2026-09-12 核对）

| 阶段 | 状态 |
|---|---|
| Phase0 / 0.5 / 1 | 历史已完成并归档，不等于本轮全功能重验 |
| Phase1.5 | 历史角色UAT方式获批；最终实际大屏验收仍须按当前版本验证 |
| Phase2 / 3 / 4 | 历史已归档；新增审阅发现跨章控制仅候选、正式目录消费未闭合，需回修Phase3；无坐标仍不画红框 |
| Phase5 | 事实/事件/用药/Profile实现存在，增量完整性和原件QC未通过；claims_complete=false |
| Phase5.5 | 持久双读、覆盖、规范化、判断检索/复核、相关UI已接线；新指定组合GLM high + MTPLX xhigh尚待统一配置与真实预检；关联质量、有效金标及临床验收未通过 |
| Phase6 | 提前实现实时审核投影/工作台；存在误绑定、缺口/结论不一致及子条款身份风险；未完成正式ReviewRun/FinalAssessment链 |
| Phase7 | 打印视图/基础合同已有；正式Action闭环、冻结报告、批量未验收；当前库相应正式实体无实例 |
| Phase8 / 9 | 未完成新项目留出全链临床验证和正式交付 |

“历史归档”不阻止修复真实回归；当前工程报告是现况证据，不能把数量或历史截图标题作为临床完成依据。阶段状态在实质性里程碑回写，不新增重复checkpoint。

### 当前实施顺序与约束

2026-09-15界面对照补充：已只读试用用户指定外部演示站，观察及对本地源码的有限对照见 `artifacts/reference-ui-20260915/OBSERVATIONS.md`。T6沿用紧凑身份/节点/方案版本、要求-依据-原件同屏、增量资料差异、历史适用性与当前结果分开；不引入演示站多账号审批。查看器已补加载/失败提示及所选文件页码，保留原件定位几何；这些是源码改进，不是本地视觉验收。统一浏览器与全链测试按最新用户要求后置至完整构建后。T3/R05继续，不因本次对照另建暂停点。

T0通用模型/预算配置 → T1事实-谓词绑定与统一判定/研究者判断适用性 → T2组件/原件身份贯穿 → T3跨章控制正式发布及累计档案 → T4 Phase5/5.5真实指定组合、有效金标/原件/大屏验收 → T5正式审核/Action/冻结报告 → T6留出项目及性能视觉 → T7交付清理。

当前两主读是部署选择，不是永久临床模型白名单；新模型应仅经能力/传输适配，不改疾病/方案/药物规则和采信标准。当前不得自动恢复Gemini、MiniMax、DeepSeek或手写第三读。新语义任务输出起始65536、一次length重试至131072，按厂商共享思考/正文物理限制核验，不静默下调。详细横评后置，但当前组合的真实质量验证不省略。

两轮辅助复核及用药分项隔离扩测已获准；未批准自动事实改写。符合完整供给范围条件的研究者判断缺失属于审核发现，不暂停要求用户确认。当前profile及projection存在缺陷，不能执行旧handoff的宽松“例外同fact_id”补丁。

执行/会商按最新全局机制；本轮C03独立只读审阅已完成，所有者采纳/否决见工程报告。测试、真实调用、临床QC、UI与发布分开。下列Phase工作及退出要求继续有效；其中明确日期的历史数字只作沿革，不是当前完成证据。
...
2. **多模型横评：** 详细横评延后至系统构建完成。当前先以GLM-5.3-Flash high × MTPLX Flash-Next Optimized-Speed xhigh完成正式产品链路及真实资料核对；不能转移历史其他模型/配置成绩，不以延后横评替代临床验收。泛化变形测试与跨方案留出验证仍须执行。
...
4c. **手写内容核对：** 两个完整主读均输出手写，按目标、类别、文字、位置及日期核对；同一模型重复不增加独立意见，不设置第三读，不把一项判断传播全报告。
5. **模型接入：** 当前GLM high（zhipu-coding-plan）与MTPLX xhigh（mtplx-flash-next-optimized-speed），按恢复计划T0统一实际角色配置/额度，禁止复核强制降档。能力与传输层兼容多模型，临床合同不按模型改写。显式env、自有HTTP，不运行个人harness；核验视觉、请求/有效身份、额度、JSON和厂商默认采样，配置不等于实测。
...
- 路由审计证明双模型互盲、无跨模型拼接；
- 路由审计证明指定MTPLX是完整第二主读，xhigh及足额预算实际生效；两主读普通事实与手写均独立提取，不把OCR或同模型重复计作独立意见；当前不自动切第三模型；
