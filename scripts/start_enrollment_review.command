...
DECONSTRUCT_ROUTE_MODE="$(printf '%s' "${DECONSTRUCT_ROUTE_MODE:-graded}" | tr '[:upper:]' '[:lower:]')"
DECONSTRUCT_BACKEND="${DECONSTRUCT_BACKEND:-$(dotenv_value DECONSTRUCT_BACKEND)}"
DECONSTRUCT_BACKEND="$(printf '%s' "${DECONSTRUCT_BACKEND:-mtplx}" | tr '[:upper:]' '[:lower:]')"
DECONSTRUCT_MODEL="${DECONSTRUCT_MODEL:-$(dotenv_value DECONSTRUCT_MODEL)}"
DECONSTRUCT_MODEL="${DECONSTRUCT_MODEL:-$MTPLX_MODEL}"
DECONSTRUCT_REASONING_EFFORT="${DECONSTRUCT_REASONING_EFFORT:-$(dotenv_value DECONSTRUCT_REASONING_EFFORT)}"
DECONSTRUCT_REASONING_EFFORT="$(printf '%s' "${DECONSTRUCT_REASONING_EFFORT:-medium}" | tr '[:upper:]' '[:lower:]')"
DECONSTRUCT_GLM_PROVIDER="${DECONSTRUCT_GLM_PROVIDER:-$(dotenv_value DECONSTRUCT_GLM_PROVIDER)}"
DECONSTRUCT_GLM_PROVIDER="$(printf '%s' "${DECONSTRUCT_GLM_PROVIDER:-zhipu-coding-plan}" | tr '[:upper:]' '[:lower:]')"
...
DECONSTRUCT_FALLBACK_DEEPSEEK_MODEL="${DECONSTRUCT_FALLBACK_DEEPSEEK_MODEL:-deepseek-v4-flash}"
...
port_is_free() {
  __omp_shell("/usr/sbin/lsof -nP -iTCP:\"$1\" -sTCP:LISTEN >/dev/null 2>&1")
}
