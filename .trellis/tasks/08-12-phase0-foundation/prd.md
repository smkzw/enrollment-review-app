...
- [ ] 冻结清单记录源文件、决定、错误回归案例、运行时与测试证据。
- [ ] V2 目录存在，依赖方向有文档和最小导入检查。
- [ ] 自动化测试证明 V2 操作不会修改 legacy 项目与报告。
- [ ] 依赖决策记录含开源许可证、版本、选择/拒绝理由和回滚方案。
- [ ] 清理 manifest 完成；仅清理确认可再生且不承担回归价值的缓存/过程文件。
- [ ] Phase 0 独立 checker 验证基线、隔离和清理边界，Codex 接受后才进入 Phase 0.5。
