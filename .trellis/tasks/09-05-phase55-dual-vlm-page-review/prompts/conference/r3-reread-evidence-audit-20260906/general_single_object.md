...
- Agent/provider/model assigned by Codex: `pi` / `cursor` / `default`
...
- Runner-managed report path: `runs/conference/r3-reread-evidence-audit-20260906/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `.trellis/tasks/09-05-phase55-dual-vlm-page-review/context/r3-reread-evidence-audit-20260906_conference_context.md`
- `.trellis/tasks/09-05-phase55-dual-vlm-page-review/plans/codex_main_venue_r3-reread-evidence-audit-20260906.md`

Your workdir is the phase5 worktree, not the packet directory. Obey the explicit read-only source allowlist in conference_context. Do not write any files, do not call clinical models, do not read credentials, do not start other agents. Report actual runtime model identity if the adapter exposes it; default is a selector, not verified identity. Output only the complete report; no third source read or clinical ruling is requested.
...
Objective:
只读审查两轮冲突复核隔离证据、严格事实计分原语和产品结束状态处理；提出最小必要修订及是否具备自动采信接入依据，不作临床签收、不改文件、不调用产品模型、不读取外部原始临床资料或凭据。
...
Output schema:
1. `# Conference Participant Output: r3-reread-evidence-audit-20260906 - general_single_object`
2. `## Boundary Check`
